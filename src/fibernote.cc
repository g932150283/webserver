...
协程切换：
执行完毕后（无论是正常结束还是异常终止），MainFunc会释放当前协程的智能指针并调用swapOut，
这样做是为了将控制权交回给协程调度器或主线程，允许其他协程运行。
...
/**
 * 执行当前协程绑定的任务函数，捕获并处理任何异常，然后更新协程的状态。
 * 
 * 功能:
 * - 从协程管理器中获取当前协程的智能指针。
 * - 执行绑定到当前协程的任务函数。
 * - 捕获执行过程中抛出的标准异常和其他未知异常。
 * - 更新协程状态为TERM（终止）或EXCEPT（异常）。
 * - 在任务执行完毕后，确保协程资源被正确释放并记录日志。
 * 
 * 参数: 无。
 * 
 * 返回值: void。
...
void Fiber::CallerMainFunc() {
    ...
    WEBSERVER_ASSERT(cur); // 确认获取的智能指针非空。

    try {
        cur->m_cb(); // 尝试执行绑定到当前协程的任务函数。
        cur->m_cb = nullptr; // 任务执行完毕，清空任务函数，防止重复执行。
        cur->m_state = TERM; // 更新协程状态为TERM（终止）。
    } catch (std::exception& ex) {
        cur->m_state = EXCEPT; // 捕获到标准异常，更新协程状态为EXCEPT（异常）。
        WEBSERVER_LOG_ERROR(g_logger) << "Fiber Except: " << ex.what()
            << " fiber_id=" << cur->getId() // 记录异常信息和协程ID。
            << std::endl
            << webserver::BacktraceToString(); // 记录当前调用栈信息。
    } catch (...) {
        cur->m_state = EXCEPT; // 捕获到未知异常，更新协程状态为EXCEPT（异常）。
        WEBSERVER_LOG_ERROR(g_logger) << "Fiber Except"
            << " fiber_id=" << cur->getId() // 记录异常信息和协程ID。
            << std::endl
            << webserver::BacktraceToString(); // 记录当前调用栈信息。
    ...
    auto raw_ptr = cur.get(); // 获取协程对象的原始指针，准备释放智能指针。
    cur.reset(); // 释放智能指针，减少引用计数。
    raw_ptr->back(); // 通知协程管理器当前协程任务执行完毕或发生异常。
    WEBSERVER_ASSERT2(false, "never reach fiber_id=" + std::to_string(raw_ptr->getId())); // 断言失败，此处代码理论上不应被执行，用于调试。
}

/*
在讨论call, back, swapIn, 和 swapOut这几个方法之前，我们先理解一下它们所处的上下文——协程（Fiber）在一个多任务协作式调度环境中的切换机制。
协程提供了一种比线程更轻量级的并发执行单位，能够在用户空间进行快速切换，无需内核的介入，从而减少上下文切换的开销。

call 和 back

call 方法：
是从主协程（或其他协程）切换到当前协程的方法。当调用 call 时，当前的执行上下文（包括调用它的协程）被保存，
并切换到了目标协程（即调用 call 方法的协程）的上下文中执行。这通常用于启动或恢复一个协程的执行。

back 方法：
与 call 相反，它是用来从当前协程切换回之前的协程（通常是主协程或者调度协程）。
back 方法在当前协程完成其任务或需要暂时让出执行权时调用，以便系统可以进行其他协程的调度。

swapIn 和 swapOut

swapIn 方法：
这个方法的作用也是将执行上下文切换到当前协程，类似于 call，但是它通常用在协程调度器中，当调度器决定执行某个协程时使用。
swapIn 通常涉及到更底层的上下文保存和恢复操作，以及更新协程状态为执行状态（EXEC）。

swapOut 方法：
与 swapIn 相对应，当当前协程需要被挂起，让出执行权时调用。swapOut 方法将执行上下文从当前协程切换回协程调度器或主协程，
同时保存当前协程的上下文以便未来恢复执行。

区别与联系

用途区别：
call 和 back 更多地用于协程间的直接切换，比如从主协程手动切换到另一个协程或返回。
而 swapIn 和 swapOut 通常被协程的调度器使用，用于更广泛的协程调度逻辑中，如在多个协程间进行调度和切换。

执行上下文：
所有这些方法都涉及到执行上下文的保存和恢复，但 call 和 back 更倾向于手动控制协程之间的切换，swapIn 和 swapOut 则是调度器在管理协程时使用。

状态管理：
swapIn 和 swapOut 在切换执行上下文的同时，还可能涉及到协程状态的管理（如将协程状态设置为执行中、挂起等），而 call 和 back 主要关注于协程的切换。

总的来说，这些方法共同为协程提供了一套灵活的执行流程控制机制，既支持手动控制协程之间的直接切换，也支持通过调度器进行复杂的协程调度和管理。
