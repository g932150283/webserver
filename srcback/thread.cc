 ...
 * @param name 新线程的名称。
 * 这个构造函数通过pthread_create创建了一个新线程，并将类实例的this指针作为参数传递给Thread::run静态函数，以便在新线程中执行成员函数。
 ...
 * 接着，它通过信号量通知构造函数线程已经准备就绪，可以继续执行。最后，它执行了线程任务（通过std::function<void()> cb），
 * 任务完成后线程将结束。
 */
