...
void test_socket() {
    ...
    //    }
    //}
    webserver::IPAddress::ptr addr = webserver::Address::LookupAnyIPAddress("www.baidu.com");
    if(addr) {
...
void test2() {
    webserver::IPAddress::ptr addr = webserver::Address::LookupAnyIPAddress("www.baidu.com:80");
    if(addr) {
        WEBSERVER_LOG_INFO(g_looger) << "get address: " << addr->toString();
    } else {
        WEBSERVER_LOG_ERROR(g_looger) << "get address fail";
